<x-app-layout>
    <div class="bg-gradient-to-br from-[#313647] to-[#435663]">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex justify-between items-center">
                <h1 class="text-xl sm:text-2xl font-bold text-[#FFF8D4]">{{ $playerRole->label_en }}</h1>
                @auth
                    <a href="{{ route('player-roles.edit', $playerRole) }}" class="inline-flex items-center px-6 py-3 bg-[#A3B087] text-[#313647] font-semibold rounded-lg hover:bg-[#FFF8D4]">
                        Edit
                    </a>
                @endauth
            </div>
    ...
    <div class="bg-[#FFF8D4] py-4">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <span class="font-medium text-[#313647]">{{ $playerRole->identifier }}</span>
        </div>
    </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg border border-gray-200">
                <div class="p-6">
                    ...
                    @if ($playerRole->description_en)
                        <div class="mb-8">
                            <h3 class="text-lg font-semibold text-[#313647] mb-2">Description</h3>
                            <p class="text-[#435663]">{{ $playerRole->description_en }}</p>
                        </div>
                    @endif

                    @if ($playerRole->games->count() > 0)
                        <div class="mb-8">
                            <h3 class="text-lg font-semibold text-[#313647] mb-4 border-b border-[#A3B087] pb-2">Games ({{ $playerRole->games->count() }})</h3>
                    ...
                    <div class="mt-6">
                        <a href="{{ route('player-roles.index') }}" class="text-[#435663] hover:text-[#313647]">← Back to Player Roles</a>
                    </div>
                </div>
            </div>
        </div>
